...
# Allow running as a script: `python src/selfloop/abstract_mountain_car_with_self_loop_removal.py`
...
from utils.ground_truth_cache import (
    build_gt_cache_path,
    load_gt_cache,
    save_gt_cache,
)
from utils.log_utils import PipelineLogger, build_method_rollup_blocks, timed_call
from utils.model_checking_tools import MountainCarModelChecker
...
from systems.mountain_car import MountainCarSystem
from utils.self_loop import *
...
if __name__ == '__main__':
    ...
    run_root = Path('runs') / 'selfloop' / 'mountain_car'
    ...
    if ARGS['run_aabb']:
        ...
        if ARGS.get('refine_aabb_self_loops', False):
            ...
            logger.record_runtime(STAGE_TRANSITIONS, 'AABB reach_set self-loop refinement', sl_cpu_dt, sl_wall_dt)
            logger.metrics('AABB reach_set Self-loop Refinement', [
                ('Candidates', sl_stats['candidates']),
                ('Removed', sl_stats['removed']),
                ('Kept', sl_stats['kept']),
                ('Stalled', sl_stats['stalled']),
            ])
            logger.runtime_line(sl_cpu_dt, sl_wall_dt, label='AABB reach_set self-loop refinement')

        transition_results["AABB_refined_reach_set"] = make_transition_map_total(aabb_shrink_working)
        ...
        loops_shrink = count_self_loops(transition_results["AABB_refined_reach_set"])
        loops_sample = count_self_loops(transition_results["AABB_refined_sample_exit"])

        print(f"AABB self-loops (raw):        {loops_raw}")
        print(f"AABB self-loops (refined reach_set):   {loops_shrink} (removed {loops_raw - loops_shrink})")
        print(f"AABB self-loops (refined sample):   {loops_sample} (removed {loops_raw - loops_sample})")

        transition_summary_blocks.append(
            ('Transition Summary - AABB (refined reach_set)', summarize_transition_map(transition_results['AABB_refined_reach_set']))
        )
        transition_summary_blocks.append(
    ...
    if ARGS['run_poly']:
        ...
        if ARGS.get('refine_poly_self_loops', False):
            ...
            logger.record_runtime(STAGE_TRANSITIONS, 'POLY reach_set self-loop refinement', sl_cpu_dt, sl_wall_dt)
            logger.metrics('POLY reach_set Self-loop Refinement', [
                ('Candidates', sl_stats['candidates']),
                ('Removed', sl_stats['removed']),
                ('Kept', sl_stats['kept']),
                ('Stalled', sl_stats['stalled']),
            ])
            logger.runtime_line(sl_cpu_dt, sl_wall_dt, label='POLY reach_set self-loop refinement')

        transition_results["POLY_refined_reach_set"] = make_transition_map_total(poly_shrink_working)
        ...
        loops_shrink = count_self_loops(transition_results["POLY_refined_reach_set"])
        loops_sample = count_self_loops(transition_results["POLY_refined_sample_exit"])

        print(f"POLY self-loops (raw):        {loops_raw}")
        print(f"POLY self-loops (refined reach_set):   {loops_shrink} (removed {loops_raw - loops_shrink})")
        print(f"POLY self-loops (refined sample):   {loops_sample} (removed {loops_raw - loops_sample})")

        transition_summary_blocks.append(
            ('Transition Summary - POLY (refined reach_set)', summarize_transition_map(transition_results['POLY_refined_reach_set']))
        )
        transition_summary_blocks.append(
    ...
    if ARGS['run_sample']:
        ...
        if ARGS.get('refine_sample_self_loops', False):
            (sample_shrink_working, sl_stats), sl_cpu_dt, sl_wall_dt = timed_call(
                refine_aabb_self_loops_by_shrink,   # same reach_set certificate
            ...
            logger.record_runtime(STAGE_TRANSITIONS, 'SAMPLE reach_set self-loop refinement', sl_cpu_dt, sl_wall_dt)
            logger.metrics('SAMPLE reach_set Self-loop Refinement', [
                ('Candidates', sl_stats['candidates']),
                ('Removed', sl_stats['removed']),
                ('Kept', sl_stats['kept']),
                ('Stalled', sl_stats['stalled']),
            ])
            logger.runtime_line(sl_cpu_dt, sl_wall_dt, label='SAMPLE reach_set self-loop refinement')

        transition_results["SAMPLE_refined_reach_set"] = make_transition_map_total(sample_shrink_working)
        ...
        loops_shrink = count_self_loops(transition_results["SAMPLE_refined_reach_set"])
        loops_sample_exit = count_self_loops(transition_results["SAMPLE_refined_sample_exit"])

        print(f"SAMPLE self-loops (raw):         {loops_raw}")
        print(f"SAMPLE self-loops (refined reach_set):    {loops_shrink} (removed {loops_raw - loops_shrink})")
        print(f"SAMPLE self-loops (refined sample-exit):{loops_sample_exit} (removed {loops_raw - loops_sample_exit})")

        transition_summary_blocks.append(
            ('Transition Summary - SAMPLE (refined reach_set)', summarize_transition_map(transition_results['SAMPLE_refined_reach_set']))
        )
        transition_summary_blocks.append(
    ...
    if ARGS['build_kripke'] and transition_results:
        ...
        for method_name, transition_map in transition_results.items():
            ...
            kripke_stats_results[method_name] = kripke_stats
    ...
    rollup_blocks = build_method_rollup_blocks(
        method_names=[
        k for k in (
            "AABB_raw", "AABB_refined_reach_set", "AABB_refined_sample_exit",
            "POLY_raw", "POLY_refined_reach_set", "POLY_refined_sample_exit",
            "SAMPLE_raw", "SAMPLE_refined_reach_set", "SAMPLE_refined_sample_exit",
        )
        ...
        kripke_stats_by_method=kripke_stats_results,
    )
    if rollup_blocks:
