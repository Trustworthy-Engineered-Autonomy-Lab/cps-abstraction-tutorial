"""
Synthetic abstraction pipeline script.
...
# Allow running as a script: `python src/selfloop/abstract_synthetic_with_self_loop_removal.py`
...
from utils.model_checking_tools import SyntheticModelChecker
...
from systems.synthetic import SyntheticSystem
...
def compare_transition_maps(name_a, map_a, name_b, map_b):
    if len(map_a) != len(map_b):
        return [
            ('Status', 'Different number of states; cannot compare.'),
        ]
...
def _format_cell(cell):
    return f"[{cell[0]:.2f}, {cell[1]:.2f}] x [{cell[2]:.2f}, {cell[3]:.2f}]"
...
if __name__ == '__main__':
    ...
    run_root = Path('runs') / 'selfloop' / 'synthetic'
    ...
    logger.metrics(
        ...
        [
            ('Grid Shape', f"({n1}, {n2})"),
            ('Bounds X1', f"[{x1_min}, {x1_max}]"),
            ('Bounds X2', f"[{x2_min}, {x2_max}]"),
    ...
    if ARGS['run_aabb']:
        ...
        transition_results["AABB_refined_reach_set"] = aabb_shrink_working
        transition_summary_blocks.append(
    ...
    if ARGS['run_poly']:
        ...
        transition_results["POLY_refined_reach_set"] = poly_shrink_working
        transition_summary_blocks.append(
    ...
    if ARGS['run_sample']:
        ...
        transition_results["SAMPLE_refined_reach_set"] = sample_shrink_working
        transition_summary_blocks.append(
    ...
    if ARGS['plot_transitions'] or ARGS['plot_false_negative_map'] or ARGS['plot_grid']:
        ...
        if ARGS['plot_grid']:
            ...
            plot_grid_abstraction(
                ...
                title='Uniform Rectilinear Abstraction (Synthetic)',
                show=False,
                system=system,
            )
            plots_saved += 1
