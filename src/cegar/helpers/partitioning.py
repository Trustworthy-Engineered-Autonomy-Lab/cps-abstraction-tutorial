import numpy as np
from cegar.helpers.math_utils import (
...
def compute_transitions_sample(
    ...
    """
    Computes transitions by Monte Carlo sampling inside each abstract cell.

    For each source cell, this method samples concrete states uniformly, steps the
    dynamics, and records successor abstract cells containing the stepped states.
    
    Args:
        grid_ctx: grid context dict returned by generate_grid(...)
        system: System object with a vectorized step() method
        n_samples: number of sampled concrete states per abstract cell
        rng_seed: optional random seed for reproducibility
        batch_cells: optional number of source cells per batch; auto-chosen if None
    ...
    mins = grid_ctx['mins']
    widths = grid_ctx['widths']

    if batch_cells is None:
        # Keep sampled-state memory around ~64MB by default.
        bytes_per_state = max(1, dims) * 8
        est_per_cell = max(1, n_samples) * bytes_per_state
        batch_cells = max(1, int((64 * 1024 * 1024) // est_per_cell))
        batch_cells = min(batch_cells, n_cells)
    else:
    ...
    for start in range(0, n_cells, batch_cells):
        end = min(start + batch_cells, n_cells)
        b = end - start

        # Uniform samples in each cell: shape (b, n_samples, D)
        u = rng.random((b, n_samples, dims))
        sampled_states = mins[start:end, None, :] + u * widths[start:end, None, :]
        flat_sampled = sampled_states.reshape(-1, dims)

        flat_next = np.asarray(system.step(flat_sampled), dtype=float)

        src_ids = np.repeat(np.arange(start, end, dtype=np.int64), n_samples)
        valid = np.ones(flat_next.shape[0], dtype=bool)
        coords = np.empty((flat_next.shape[0], dims), dtype=np.int64)
        ...
        for d, lines in enumerate(grid_lines):
            if periodic_theta and d == 2:
                ...
                coords[:, d] = np.clip(idx, 0, grid_dims[d] - 1)
                continue
        ...
        dst_flat = coords[valid] @ strides
        src_valid = src_ids[valid]

        # Deduplicate per batch before Python set updates.
        packed = src_valid * n_cells + dst_flat
        uniq = np.unique(packed)
        src_uniq = uniq // n_cells
        dst_uniq = uniq % n_cells

        for s, d in zip(src_uniq, dst_uniq):
            transition_map[int(s)].add(int(d))
