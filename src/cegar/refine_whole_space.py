...
from cegar.abstraction import Abstraction, Rect
from cegar.cegar_loop import ctl_get_counterexample_lasso, validate_lasso_by_set_propagation
...
def main() -> None:
    sysmod = sys.argv[1] if len(sys.argv) > 1 else "systems.synthetic"
    ...
    plot_classification(
        ...
        from cegar.abstraction import Rect, RectPartition
        from cegar.cegar_loop import ctl_get_counterexample_lasso, validate_lasso_by_set_propagation
        from cegar.krish_abstraction import KrishAbstraction
    main()
