...
class PipelineLogger:
    ...
    def metrics_side_by_side(
        ...
        *,
        ...
        """
        Render multiple 2-column metric tables side-by-side.

        By default, blocks are automatically wrapped into multiple rows based on the
        current terminal width to avoid long lines being soft-wrapped by the terminal
        (which tends to visually "smoosh" neighboring tables together).
        ...
        Args:
            ...
            wrap: if True, wrap blocks into multiple rows to fit within max_width.
            max_width: terminal width override (columns). Defaults to detected width.
            row_gap_lines: blank lines between wrapped rows.
        """
        if max_width is None:
            # Fallback width is conservative; users can override via max_width.
            max_width = shutil.get_terminal_size(fallback=(120, 24)).columns

        max_width = int(max_width) if max_width is not None else 120
        gap = int(gap)
        row_gap_lines = int(row_gap_lines)
        ...
        def _pack_rows(block_specs):
            ...
            current = []
            current_width = 0
            for lines, width, header_rows in block_specs:
                add_gap = gap if current else 0
                if current and (current_width + add_gap + width) > max_width:
                    rows.append(current)
                    current = [(lines, width, header_rows)]
                    current_width = width
                else:
                    current.append((lines, width, header_rows))
                    current_width += add_gap + width
            if current:
                rows.append(current)
            return rows

        spacer = " " * gap
        packed_rows = _pack_rows(rendered_blocks)
        for row_idx, row_blocks in enumerate(packed_rows):
            max_lines = max(len(lines) for lines, _, _ in row_blocks)
            padded = []
            for lines, width, header_rows in row_blocks:
            ...
            if row_gap_lines > 0 and row_idx < (len(packed_rows) - 1):
                for _ in range(row_gap_lines):
                    print("")
