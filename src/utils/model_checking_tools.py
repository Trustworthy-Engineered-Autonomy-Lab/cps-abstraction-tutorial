...
from utils.math_utils import build_hyperrectangle_vertices
...
class SyntheticModelChecker(BaseModelChecker):
    ...
    def get_gt_reach_regions(self, domain, grid_resolution=100, max_steps=10_000, verbose=False):
        ...
        x_lo = x_edges[flat_i]
        x_hi = x_edges[flat_i + 1]
        y_lo = y_edges[flat_j]
        y_hi = y_edges[flat_j + 1]

        verts = np.empty((total, 4, 2), dtype=float)
        verts[:, 0, :] = np.column_stack((x_lo, y_lo))
        verts[:, 1, :] = np.column_stack((x_lo, y_hi))
        verts[:, 2, :] = np.column_stack((x_hi, y_hi))
        verts[:, 3, :] = np.column_stack((x_hi, y_lo))
        ...
        for _ in range(int(max_steps)):
            ...
            active_verts = verts[active_idx]
            hits_oob = np.any(
                (active_verts[:, :, 0] < x_min)
                | (active_verts[:, :, 0] > x_max)
                | (active_verts[:, :, 1] < y_min)
                | (active_verts[:, :, 1] > y_max),
                axis=1,
            )
            if np.any(hits_oob):
                fail_idx = active_idx[hits_oob]
                labels[fail_idx] = 0
                active[fail_idx] = False

            survivors = active_idx[~hits_oob]
            if survivors.size == 0:
                continue

            survivor_verts = verts[survivors]
            dist_sq = np.sum((survivor_verts - self.goal_center[None, None, :]) ** 2, axis=2)
            in_goal = np.all(dist_sq <= goal_radius_sq, axis=1)
            if np.any(in_goal):
                goal_idx = survivors[in_goal]
                labels[goal_idx] = 1
                active[goal_idx] = False

            step_idx = survivors[~in_goal]
            if step_idx.size:
                stepped = np.asarray(self.system.step(verts[step_idx].reshape(-1, 2)), dtype=float)
                verts[step_idx] = stepped.reshape(-1, 4, 2)

        if verbose:
            for report in range(0, total, 10_000):
                print(f"    > Processed {report} / {total} regions...")

        label_lut = np.asarray(['fail', 'goal', 'unk'], dtype=object)
        label_values = label_lut[labels]
        gt_reach_regions = {
            (int(i), int(j)): str(label_values[idx])
            for idx, (i, j) in enumerate(zip(flat_i, flat_j))
        }
...
class MountainCarModelChecker(BaseModelChecker):
    ...
    def get_gt_reach_regions(self, domain, grid_resolution=100, max_steps=10_000, verbose=False):
        ...
        i_grid, j_grid = np.meshgrid(
            np.arange(np_cells, dtype=np.int64),
            np.arange(nv_cells, dtype=np.int64),
            indexing='ij',
        )
        flat_i = i_grid.reshape(-1)
        flat_j = j_grid.reshape(-1)
        ...
        p_lo = p_edges[flat_i]
        p_hi = p_edges[flat_i + 1]
        v_lo = v_edges[flat_j]
        v_hi = v_edges[flat_j + 1]

        verts = np.empty((total, 4, 2), dtype=float)
        verts[:, 0, :] = np.column_stack((p_lo, v_lo))
        verts[:, 1, :] = np.column_stack((p_lo, v_hi))
        verts[:, 2, :] = np.column_stack((p_hi, v_hi))
        verts[:, 3, :] = np.column_stack((p_hi, v_lo))

        labels = np.full(total, 'unk', dtype=object)
        active = np.ones(total, dtype=bool)
        ...
        for _ in range(int(max_steps)):
            ...
            active_verts = verts[active_idx]
            in_goal = np.all(active_verts[:, :, 0] >= self.goal_position_min, axis=1)
            if np.any(in_goal):
                goal_idx = active_idx[in_goal]
                labels[goal_idx] = 'goal'
                active[goal_idx] = False

            step_idx = active_idx[~in_goal]
            if step_idx.size:
                stepped = np.asarray(self.system.step(verts[step_idx].reshape(-1, 2)), dtype=float)
                verts[step_idx] = stepped.reshape(-1, 4, 2)
...
class UnicycleModelChecker(BaseModelChecker):
    ...
    def get_gt_reach_regions(self, domain, grid_resolution=100, max_steps=10_000, verbose=False):
        ...
        goal_radius_sq = self.goal_radius * self.goal_radius
        obs_centers = np.asarray(self.obstacle_centers[:, :2], dtype=float)
        obs_radii_sq = np.asarray(self.obstacle_radii * self.obstacle_radii, dtype=float)

        chunk_n_cells = 8_192
        next_report = 0
        for start in range(0, total, chunk_n_cells):
            end = min(start + chunk_n_cells, total)
            b = end - start

            flat = np.arange(start, end, dtype=np.int64)
            ii, jj, kk = np.unravel_index(flat, (nx, ny, nt))
            ...
            verts = np.empty((b, 8, 3), dtype=float)
            verts[:, 0, :] = np.column_stack((x_lo, y_lo, t_lo))
            verts[:, 1, :] = np.column_stack((x_lo, y_hi, t_lo))
            verts[:, 2, :] = np.column_stack((x_hi, y_hi, t_lo))
            verts[:, 3, :] = np.column_stack((x_hi, y_lo, t_lo))
            verts[:, 4, :] = np.column_stack((x_lo, y_lo, t_hi))
            verts[:, 5, :] = np.column_stack((x_lo, y_hi, t_hi))
            verts[:, 6, :] = np.column_stack((x_hi, y_hi, t_hi))
            verts[:, 7, :] = np.column_stack((x_hi, y_lo, t_hi))

            # 0: fail, 1: goal, 2: unk
            labels = np.full(b, 2, dtype=np.int8)
            ...
            for _ in range(max_steps):
                ...
                active_verts = verts[active_idx]
                active_xy = active_verts[:, :, :2]

                obstacle_hit = np.zeros(active_idx.size, dtype=bool)
                for center, radius_sq in zip(obs_centers, obs_radii_sq):
                    diff = active_xy - center[None, None, :]
                    dist_sq = np.sum(diff * diff, axis=2)
                    obstacle_hit |= np.any(dist_sq <= radius_sq, axis=1)

                if np.any(obstacle_hit):
                    fail_idx = active_idx[obstacle_hit]
                    labels[fail_idx] = 0
                    active[fail_idx] = False

                remaining_idx = active_idx[~obstacle_hit]
                if remaining_idx.size == 0:
                    continue

                remaining_verts = verts[remaining_idx]
                remaining_xy = remaining_verts[:, :, :2]

                goal_dist_sq = np.sum((remaining_xy - self.goal_center[None, None, :]) ** 2, axis=2)
                goal_hit = np.all(goal_dist_sq <= goal_radius_sq, axis=1)
                if np.any(goal_hit):
                    goal_idx = remaining_idx[goal_hit]
                    labels[goal_idx] = 1
                    active[goal_idx] = False

                step_idx = remaining_idx[~goal_hit]
                if step_idx.size == 0:
                    continue

                step_xy = verts[step_idx][:, :, :2]
                oob_xy = np.any(
                    (step_xy[:, :, 0] < x_min)
                    | (step_xy[:, :, 0] > x_max)
                    | (step_xy[:, :, 1] < y_min)
                ...
                if np.any(oob_xy):
                    fail_idx = step_idx[oob_xy]
                    labels[fail_idx] = 0
                    active[fail_idx] = False

                step_idx = step_idx[~oob_xy]
                if step_idx.size:
                    stepped = np.asarray(self.system.step(verts[step_idx].reshape(-1, 3)), dtype=float)
                    verts[step_idx] = stepped.reshape(-1, 8, 3)

            label_lut = np.asarray(['fail', 'goal', 'unk'], dtype=object)
            label_values = label_lut[labels]
            for idx in range(b):
                gt_reach_regions[(int(ii[idx]), int(jj[idx]), int(kk[idx]))] = str(label_values[idx])

            if verbose:
                while next_report < end:
                    print(f"    > Processed {next_report} / {total} regions...")
                    next_report += 10_000
