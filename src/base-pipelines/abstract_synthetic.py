...
# Allow running as a script: `python src/abstraction/abstract_synthetic.py`
...
if __name__ == '__main__':
    ...
    run_root = Path('runs') / 'base' / 'synthetic'
    ...
    if ARGS['run_aabb']:
        transition_results['AABB'], ta_cpu_dt, ta_wall_dt = timed_call(
            compute_transitions_AABB,
            grid_ctx,
            system,
        )
        logger.record_runtime(STAGE_TRANSITIONS, 'AABB transition build', ta_cpu_dt, ta_wall_dt)
        transition_summary_blocks.append(('Transition Summary - AABB', summarize_transition_map(transition_results['AABB'])))
        transition_runtime_lines.append(('AABB transition build', ta_cpu_dt, ta_wall_dt))

    if ARGS['run_poly']:
        transition_results['POLY'], tp_cpu_dt, tp_wall_dt = timed_call(
            compute_transitions_poly,
            grid_ctx,
            system,
        )
        logger.record_runtime(STAGE_TRANSITIONS, 'POLY transition build', tp_cpu_dt, tp_wall_dt)
        transition_summary_blocks.append(('Transition Summary - POLY', summarize_transition_map(transition_results['POLY'])))
        transition_runtime_lines.append(('POLY transition build', tp_cpu_dt, tp_wall_dt))

    if ARGS['run_sample']:
        transition_results['SAMPLE'], ts_cpu_dt, ts_wall_dt = timed_call(
            compute_transitions_sample,
            grid_ctx,
            system,
        ...
        transition_summary_blocks.append(('Transition Summary - SAMPLE', summarize_transition_map(transition_results['SAMPLE'])))
    ...
    pair_names = [('AABB', 'POLY'), ('POLY', 'SAMPLE'), ('AABB', 'SAMPLE')]
    ...
    rollup_blocks = build_method_rollup_blocks(
        method_names=[name for name in ('AABB', 'POLY', 'SAMPLE') if name in transition_results],
